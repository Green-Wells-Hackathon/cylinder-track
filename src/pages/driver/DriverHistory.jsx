...
export default function DriverHistory() {
  ...
  useEffect(() => {
    const unsub = getOrdersRealtime(all => {
      const delivered = all.filter(o =>
        (Array.isArray(o.orderFull?.status) && o.orderFull.status.includes('delivered')) ||
        (o.orderFull?.order_completed === true) ||
        (String(o.orderFull?.status)?.toLowerCase().includes('delivered')) ||
        (String(o.status).toLowerCase() === 'delivered')
      );
      const mapped = delivered.map(d => ({ ...d, status: 'Delivered' }));
      setOrders(mapped);
    ...
    <div className="driver-page">
      <aside className="driver-sidebar">
        ...
        <nav className="driver-nav">
          ...
          <Link to="/driver/history" className="active"><FaHourglassHalf /> History</Link>
        </nav>
      </aside>
